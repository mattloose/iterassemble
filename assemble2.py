...
if __name__ == "__main__":
    ...
    for i in range(1,args.m+1):
        ...
        soapout1 = "iter" + str(i) + "_63soap";
        soapout2 = "iter" + str(i) + "_31soap";
        if os.path.exists(soapout1+".scafSeq"):
            subprocess.call("rm "+soapout1+"* "+soapout2+"* ", shell=True)
        soapout = "iter" + str(i) + "_soap";

        subprocess.call('SOAPdenovo-63mer all -s '+conf+' -K 63 -R -F -p '+str(args.t)+' -o '+soapout1, shell=True)

        subprocess.call('SOAPdenovo-63mer all -s '+conf+' -R -F -p '+str(args.t)+' -o '+soapout2, shell=True)
        ...
        with open(ref, 'w') as ins:
            for record in SeqIO.parse(passfile, "fasta"):
                ...
                if i == 1 or len(seq) < args.endsize*2:
                    ...
                    ins.write(">" + record.id + "_start\n")
                    ins.write(seq[:args.endsize] + "\n")
                    ins.write(">" + record.id + "_end\n")
                    ins.write(seq[-args.endsize:] + "\n")
